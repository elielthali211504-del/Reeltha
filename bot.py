...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8551744239:AAG2J_gpBldPcZgLJP4DBKBh7_ctgGGUZ6o")
ADMIN_ID = int(os.environ.get("ADMIN_ID", "1461573114"))
...
def load_revistas_config():
    """Carga la configuración de revistas desde archivo JSON"""
    try:
        ...
        print(f"❌ Error al cargar configuración de revistas: {e}")
        return REVISTAS

def save_revistas_config(config):
    """Guarda la configuración de revistas en archivo JSON"""
    try:
        with open(REVISTAS_CONFIG_FILE, 'w', encoding='utf-8') as f:
            json.dump(config, f, ensure_ascii=False, indent=2)
        return True
    except Exception as e:
        print(f"❌ Error al guardar configuración de revistas: {e}")
        return False

# Cargar configuración de revistas
    ...
    try:
        ...
        for user in users:
            if user.get('id') == user_id:
                print(f"⚠️ Usuario {user_id} ya existe")
                return False
        ...
        if save_authorized_users(users):
            ...
            print(f"❌ Error al guardar usuario {user_id}")
            return False
            
    except Exception as e:
        print(f"❌ Error en add_authorized_user: {e}")
        return False
    ...
    try:
        ...
        if len(users) < original_count:
            if save_authorized_users(users):
                ...
                print(f"❌ Error al guardar después de eliminar")
                return False
        else:
        ...
        print(f"❌ Error en remove_authorized_user: {e}")
        return False
...
def is_user_authorized(user_id):
    ...
    try:
        ...
        # El administrador SIEMPRE está autorizado
        if user_id == ADMIN_ID:
            return True
        ...
        print(f"❌ Error en is_user_authorized: {e}")
        return False

# Inicializar lista de usuarios autorizados
if not os.path.exists(AUTHORIZED_USERS_FILE):
    save_authorized_users([])
    print(f"📄 Archivo {AUTHORIZED_USERS_FILE} creado")
...
def authorized_only(func):
    ...
    async def wrapper(client, message):
        try:
            ...
            if not user_id:
                await message.reply("❌ No se pudo identificar al usuario.")
                return
            ...
            await message.reply(f"❌ Error de autorización: {str(e)}")
...
class RevistaUploader:
    ...
    def login(self):
        try:
            ...
            return 'Cerrar sesión' in resp.text or 'submissionId=' in resp.text
        except Exception as e:
    ...
    def get_submission_files(self):
        """Obtiene la lista de archivos subidos al envío"""
...
def fake_pdf(input_path):
    """Renombra un archivo agregando extensión .pdf"""
    try:
        ...
        print(f"✅ Archivo renombrado a PDF: {os.path.basename(fake_path)}")
        return fake_path
    except Exception as e:
        print(f"❌ Error al renombrar archivo a PDF: {e}")
        return None
...
def prepare_chunked_parts(original_path, chunk_size_bytes, work_dir):
    ...
    try:
        ...
        print(f"📦 Preparando partes para: {fname}")
        print(f"📊 Tamaño total: {total_size / 1024 / 1024:.2f} MB")
        print(f"📏 Tamaño por parte: {chunk_size_bytes / 1024 / 1024:.2f} MB")
        ...
        with open(original_path, "rb") as src:
            ...
            while offset < total_size:
                ...
                print(f"  ✅ Parte {part_idx} creada: {pdf_name} ({part_size / 1024 / 1024:.2f} MB)")
                
                part_idx += 1
                offset += len(chunk_data)
        
        print(f"📋 Total partes creadas: {len(parts_info)}")
        return parts_info
        
    except Exception as e:
        print(f"❌ Error en prepare_chunked_parts: {e}")
        return parts_info

def get_file_list():
    """Obtiene lista de TODOS los archivos en la raíz"""
    all_files = []
    try:
        ...
        print(f"❌ Error en get_file_list: {e}")
        return []
...
async def progress_bar_generic(current, total, status_msg, file_name, action="Descargando", part=None, total_parts=None, total_size=None, part_size=None):
    try:
        ...
        if part and total_parts and total_size and part_size:
            extra = (f"\n📊 Parte {part}/{total_parts} ({part_size/1024/1024:.2f} MB)"
                     f"\n📦 Total: {total_size/1024/1024:.2f} MB en {total_parts} partes")

        await status_msg.edit_text(
            f"{'⬇️' if action=='Descargando' else '⬆️'} {action}: {file_name}\n"
            f"[{bar}] {percent:.2f}%\n"
...
# =========================
# MANEJADOR DE SEÑALES PARA RENDER
# =========================
def handle_signal(signum, frame):
    print(f"\n⚠️ Señal {signum} recibida. Cerrando el bot...")
    exit(0)
...
async def start_handler(client, message):
    try:
        ...
        if user_authorized:
            revistas_text = "\n".join([f"• {key}: {data['nombre']}" for key, data in REVISTAS.items()])
            
            await message.reply(
                f"👋 **Hola, {user_name}!**\n\n"
                f"✅ **Estás autorizado para usar este bot.**\n\n"
                f"📚 **Revistas configuradas:**\n{revistas_text}\n\n"
                f"📂 **Directorio raíz:** `{ROOT_DIR}`\n"
                f"📦 **Tamaño de partes:** `{CHUNK_SIZE_MB} MB`\n\n"
                f"**Comandos disponibles:**\n"
                f"• /help - Muestra todos los comandos\n"
                f"• /ls - Ver archivos guardados\n"
                f"• /rm <número> - Borrar un archivo\n"
                f"• /deleteall - Borrar todo\n"
                f"• /up - Subir archivos a una revista\n"
                f"• /up <número> - Subir archivo específico\n"
                f"• /zips <tamaño> - Cambiar tamaño de partes\n"
                f"• /clear_rev - Limpiar archivos de revista\n"
                f"• /settings - Panel de configuración (Admin)\n\n"
                f"👨‍💻 **Desarrollador:** @Pro_Slayerr\n"
                f"🆔 **Tu ID:** `{user_id}`"
            )
        else:
        ...
        await message.reply(f"❌ Error en /start: {str(e)}")

@app.on_message(filters.command("help"))
async def help_handler(client, message):
    try:
        help_text = f"""
📚 **BOT DE GESTIÓN DE REVISTAS** - Ayuda

**COMANDOS DISPONIBLES:**

👑 **Administrador:**
• /add <id/@user> - Agregar usuario autorizado
• /ban <id> - Eliminar usuario autorizado
...
• /ls - Listar archivos guardados
• /rm <número> - Eliminar archivo específico
• /deleteall - Eliminar todos los archivos

📤 **Subida a revistas:**
• /up - Subir archivos (modo interactivo)
• /up <número> - Subir archivo específico
• /up <1,2,3> - Subir múltiples archivos
• /up <1-3> - Subir rango de archivos
• /up_<número> - Subir archivo específico (nuevo formato)
• /zips <MB> - Cambiar tamaño de partes (1-100 MB)
...
"""
        ...
        for key, data in REVISTAS.items():
            help_text += f"• {key}: {data['nombre']}\n"
        
        help_text += f"""
        
⚙️ **Configuración actual:**
• Tamaño de partes: {CHUNK_SIZE_MB} MB
• Directorio raíz: {ROOT_DIR}
• Usuarios autorizados: {len(load_authorized_users())}
...
"""
        ...
        await message.reply(f"❌ Error en /help: {str(e)}")
    ...
    try:
        ...
        if len(message.command) < 2:
            await message.reply(
                ...
                "• /add 123456789\n"
                "• /add @usuario\n\n"
                "También puedes responder a un mensaje con /add para agregar ese usuario."
            )
            return
        ...
        if message.reply_to_message:
            ...
            try:
                ...
                await message.reply("❌ User ID inválido.")
                return
            ...
            try:
                ...
                await message.reply(f"❌ No se pudo encontrar el usuario @{username}")
                return
        
        else:
            await message.reply("❌ Formato inválido. Usa: /add <user_id> o /add @username")
            return
        ...
        if target_user:
            if add_authorized_user(target_user.id, target_user.username):
                await message.reply(
                    ...
                    f"👤 **ID:** `{target_user.id}`\n"
                    f"📛 **Usuario:** @{target_user.username if target_user.username else 'N/A'}\n"
                    f"📅 **Fecha:** {time.strftime('%Y-%m-%d %H:%M:%S')}"
                )
            else:
                await message.reply(f"❌ El usuario `{target_user.id}` ya está en la lista de autorizados.")
                
    except Exception as e:
        await message.reply(f"❌ Error en /add: {str(e)}")
    ...
    try:
        ...
        if len(message.command) < 2:
            await message.reply(
                ...
                "También puedes responder a un mensaje con /ban para eliminar ese usuario."
            )
            return
        ...
        if message.reply_to_message:
            ...
            await message.reply("❌ Formato inválido. Usa: /ban <user_id>")
            return
        
        if target_user_id:
            if remove_authorized_user(target_user_id):
                await message.reply(f"✅ Usuario `{target_user_id}` eliminado de la lista de autorizados.")
            else:
                await message.reply(f"❌ El usuario `{target_user_id}` no estaba en la lista de autorizados.")
                
    except Exception as e:
        await message.reply(f"❌ Error en /ban: {str(e)}")
...
async def list_users_handler(client, message):
    try:
        ...
        if not users:
            await message.reply("📭 No hay usuarios autorizados en la lista.")
            return
        ...
        await message.reply(
            f"👥 **Usuarios autorizados** ({len(users)}):\n\n" +
            "\n".join(user_list)
        )
        
    except Exception as e:
        await message.reply(f"❌ Error en /users: {str(e)}")
...
async def list_files(client, message):
    try:
        ...
        if not all_files:
            await message.reply("📭 No hay archivos guardados.")
            return
        ...
        header = f"📂 **𝑫𝒊𝒓𝒆𝒄𝒕𝒐𝒓𝒊𝒐 𝒂𝒄𝒕𝒖𝒂𝒍:** {ROOT_DIR}/\n\n"
        header += f"⬇️ **𝑻𝒐𝒕𝒂𝒍 𝑫𝒐𝒘𝒏:** {format_file_size(total_size)}\n\n"
        ...
        for i, path in enumerate(all_files, start=1):
            ...
            listado.append(f"{i}{icon}**{file_name[:50]}**\n  ➥ ❮/up_{i}❯  ❮/rm_{i}❯ [{formatted_size}]\n")
        
        footer = f"\n⏳ **𝑻𝒊𝒆𝒎𝒑𝒐 𝑬𝒏𝒄𝒆𝒏𝒅𝒊𝒅𝒐:** {uptime}\n"
        footer += f"📦 **𝑻𝒂𝒎𝒂ñ𝒐 𝒑𝒂𝒓𝒕𝒆𝒔:** {CHUNK_SIZE_MB} MB\n"
        footer += f"👨‍💻 **𝑫𝒆𝒔𝒂𝒓𝒓𝒐𝒍𝒍𝒂𝒅𝒐𝒓:** @Pro_Slayerr"
        
        # Crear botones inline - SOLO LOS 2 BOTONES REQUERIDOS
        keyboard = [
            [
                InlineKeyboardButton("⬆️ 𝑺𝒖𝒃𝒊𝒓 𝑻𝒐𝒅𝒐", callback_data="upload_all_files"),
                InlineKeyboardButton("🗑️ 𝑬𝒍𝒊𝒎𝒊𝒏𝒂𝒓 𝑻𝒐𝒅𝒐", callback_data="confirm_delete_all_global")
            ]
        ]
        ...
        await message.reply(f"❌ Error en /ls: {str(e)}")
...
async def remove_file(client, message):
    try:
        ...
        if idx < 1 or idx > len(all_files):
            await message.reply(f"⚠️ Índice fuera de rango. Usa un número entre 1 y {len(all_files)}")
            return
        ...
        try:
            os.remove(target)
            await message.reply(f"✅ Archivo eliminado: `{file_name}`")
        except Exception as e:
            await message.reply(f"❌ Error al eliminar archivo: {str(e)}")
            
    except ValueError:
        await message.reply("❌ Número inválido. Usa: /rm <número>")
    except Exception as e:
        await message.reply(f"❌ Error en /rm: {str(e)}")

@app.on_message(filters.regex(r'^/up'))
@authorized_only
async def upload_handler(client, message):
    """Manejador para /up y /up_<número>"""
    try:
        user_id = message.from_user.id
        ...
        if not all_files:
            await message.reply("📭 No hay archivos guardados para subir.")
            return
...
async def select_revista_interactive(client, message, all_files):
    ...
    try:
        ...
        if len(all_files) > 10:
            list_text += f"\n... y {len(all_files) - 10} más\n"
        
        keyboard = []
        for revista_id, revista_info in REVISTAS.items():
            keyboard.append([
                InlineKeyboardButton(
                    f"📚 {revista_info['nombre']}",
                    callback_data=f"upload_all_{revista_id}"
                )
            ])
        
        keyboard.append([InlineKeyboardButton("❌ Cancelar", callback_data="cancel_upload")])
        reply_markup = InlineKeyboardMarkup(keyboard)
...
async def remove_file_underscore(client, message):
    ...
    try:
        ...
        try:
            ...
            await message.reply("❌ Número inválido. Usa: /rm_<número>")
            return
        ...
        await message.reply(f"❌ Error en /rm_: {str(e)}")
...
async def set_zip_size(client, message):
    ...
    try:
        ...
        if len(parts) < 2:
            await message.reply("**Uso:** /zips <tamaño_en_MB>\n**Ejemplo:** /zips 10")
            return
        ...
        if new_size < 1 or new_size > 100:
            await message.reply("⚠️ El tamaño debe estar entre 1 y 100 MB")
            return
        ...
        await message.reply("❌ Número inválido. Usa: /zips <tamaño_en_MB>")
    except Exception as e:
        await message.reply(f"❌ Error en /zips: {str(e)}")
...
async def save_file(client, message):
    try:
        ...
        status_msg = await message.reply(f"⬇️ Descargando: `{file_name}`")
        ...
        await status_msg.edit_text(f"✅ Guardado: `{file_name}` ({format_file_size(file_size)})")
        
    except Exception as e:
        await message.reply(f"❌ Error al guardar archivo: {str(e)}")
...
async def delete_all(client, message):
    try:
        keyboard = [
            [
                InlineKeyboardButton("✅ 𝑺í, 𝒆𝒍𝒊𝒎𝒊𝒏𝒂𝒓 𝒕𝒐𝒅𝒐", callback_data="confirm_delete_all"),
                InlineKeyboardButton("❌ 𝑪𝒂𝒏𝒄𝒆𝒍𝒂𝒓", callback_data="cancel_delete_all")
            ]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        ...
        await message.reply(
            f"⚠️ **¿𝑬𝒔𝒕á 𝒔𝒆𝒈𝒖𝒓𝒐 𝒅𝒆 𝒒𝒖𝒆 𝒅𝒆𝒔𝒆𝒂 𝒆𝒍𝒊𝒎𝒊𝒏𝒂𝒓 𝑻𝑶𝑫𝑶𝑺 𝒍𝒐𝒔 𝒂𝒓𝒄𝒉𝒊𝒗𝒐𝒔?**\n\n"
            f"📊 **𝑨𝒓𝒄𝒉𝒊𝒗𝒐𝒔:** {len(all_files)}\n"
            f"💾 **𝑻𝒂𝒎𝒂ñ𝒐 𝒕𝒐𝒕𝒂𝒍:** {format_file_size(total_size)}\n"
            f"📂 **𝑫𝒊𝒓𝒆𝒄𝒕𝒐𝒓𝒊𝒐:** `{ROOT_DIR}`\n\n"
            f"𝑬𝒔𝒕𝒂 𝒂𝒄𝒄𝒊ó𝒏 **𝑵𝑶** 𝒔𝒆 𝒑𝒖𝒆𝒅𝒆 𝒅𝒆𝒔𝒉𝒂𝒄𝒆𝒓.",
            reply_markup=reply_markup
        )
    except Exception as e:
        await message.reply(f"❌ Error en /deleteall: {str(e)}")

async def process_upload_parameters(client, message, param, all_files):
    """Procesa parámetros del comando /up"""
    try:
        selected_indices = []
        ...
        if not selected_indices:
            await message.reply("❌ No se encontraron archivos válidos con los índices proporcionados.")
            return
        ...
        summary = f"📦 **𝑨𝒓𝒄𝒉𝒊𝒗𝒐𝒔 𝒔𝒆𝒍𝒆𝒄𝒄𝒊𝒐𝒏𝒂𝒅𝒐𝒔** ({len(selected_files)}):\n\n"
        ...
        await message.reply(
            f"{summary}\n"
            f"📦 𝑻𝒂𝒎𝒂ñ𝒐 𝒑𝒂𝒓𝒕𝒆𝒔: {CHUNK_SIZE_MB} MB\n\n"
            f"**𝑺𝒆𝒍𝒆𝒄𝒄𝒊𝒐𝒏𝒆 𝒍𝒂 𝒓𝒆𝒗𝒊𝒔𝒕𝒂 𝒅𝒆𝒔𝒕𝒊𝒏𝒐:**",
            reply_markup=reply_markup
        )
        ...
        await message.reply(
            "❌ Formato inválido.\n\n"
            "**𝑭𝒐𝒓𝒎𝒂𝒕𝒐𝒔 𝒗á𝒍𝒊𝒅𝒐𝒔:**\n"
            "• `/up 1` - Subir archivo #1\n"
            "• `/up_1` - Subir archivo #1 (nuevo formato)\n"
            "• `/up 1,3,5` - Subir archivos 1, 3 y 5\n"
            "• `/up 1-5` - Subir archivos 1 al 5\n"
            "• `/up` - Modo interactivo"
        )
    except Exception as e:
        await message.reply(f"❌ Error en process_upload_parameters: {str(e)}")
...
async def clear_revista_handler(client, message):
    try:
        ...
        for revista_id, revista_info in REVISTAS.items():
            keyboard.append([
                InlineKeyboardButton(
                    f"🗑️ {revista_info['nombre']}",
                    callback_data=f"clear_rev_{revista_id}"
                )
            ])
        
        keyboard.append([InlineKeyboardButton("❌ Cancelar", callback_data="cancel_clear")])
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        await message.reply(
            "⚠️ **𝑨𝑫𝑽𝑬𝑹𝑻𝑬𝑵𝑪𝑰𝑨:** Esta acción eliminará 𝑻𝑶𝑫𝑶𝑺 los archivos de la revista.\n\n"
            "**𝑺𝒆𝒍𝒆𝒄𝒄𝒊𝒐𝒏𝒆 𝒍𝒂 𝒓𝒆𝒗𝒊𝒔𝒕𝒂 𝒂 𝒍𝒊𝒎𝒑𝒊𝒂𝒓:**",
            reply_markup=reply_markup
        )
        
    except Exception as e:
        await message.reply(f"❌ Error en /clear_rev: {str(e)}")
...
async def settings_handler(client, message):
    try:
        ...
        if user_id != ADMIN_ID:
            await message.reply("❌ No tienes autorización para utilizar este comando.")
            return
        
        keyboard = [
            [InlineKeyboardButton("⚙️ Configurar Revista", callback_data="config_revista")],
            [InlineKeyboardButton("📊 Ver Configuración Actual", callback_data="view_config")],
            [InlineKeyboardButton("❌ Cerrar Panel", callback_data="close_panel")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        await message.reply(
            "🔧 **𝑷𝑨𝑵𝑬𝑳 𝑫𝑬 𝑪𝑶𝑵𝑭𝑰𝑮𝑼𝑹𝑨𝑪𝑰Ó𝑵**\n\n"
            "Seleccione una opción:",
            reply_markup=reply_markup
        )
        
    except Exception as e:
        await message.reply(f"❌ Error en /settings: {str(e)}")
...
async def handle_callback(client, callback_query):
    ...
    try:
        if any(data.startswith(prefix) for prefix in ["upload_", "clear_rev_", "confirm_", "config_", "delete_all"]):
            if not is_user_authorized(user_id):
                await callback_query.answer("❌ No estás autorizado.", show_alert=True)
                return
        ...
        if data == "cancel_upload":
            ...
            await callback_query.message.edit_text("❌ Limpieza cancelada.")
            await callback_query.answer()
            ...
            os.makedirs(ROOT_DIR, exist_ok=True)
            await callback_query.message.edit_text("✅ 𝑻𝒐𝒅𝒐𝒔 𝒍𝒐𝒔 𝒂𝒓𝒄𝒉𝒊𝒗𝒐𝒔 𝒉𝒂𝒏 𝒔𝒊𝒅𝒐 𝒆𝒍𝒊𝒎𝒊𝒏𝒂𝒅𝒐𝒔.")
            await callback_query.answer()
            ...
            await callback_query.message.edit_text(
                f"⚠️ **𝑪𝒐𝒏𝒇𝒊𝒓𝒎𝒂𝒄𝒊ó𝒏 𝒇𝒊𝒏𝒂𝒍**\n\n"
                f"¿𝑬𝒔𝒕á 𝑨𝑩𝑺𝑶𝑳𝑼𝑻𝑨𝑴𝑬𝑵𝑻𝑬 𝒔𝒆𝒈𝒖𝒓𝒐 𝒅𝒆 𝒆𝒍𝒊𝒎𝒊𝒏𝒂𝒓 𝑻𝑶𝑫𝑶𝑺 𝒍𝒐𝒔 𝒂𝒓𝒄𝒉𝒊𝒗𝒐𝒔?\n\n"
                f"📊 𝑨𝒓𝒄𝒉𝒊𝒗𝒐𝒔: {len(all_files)}\n"
                f"💾 𝑻𝒂𝒎𝒂ñ𝒐 𝒕𝒐𝒕𝒂𝒍: {format_file_size(total_size)}\n\n"
                f"⚠️ 𝑬𝒔𝒕𝒂 𝒂𝒄𝒄𝒊ó𝒏 𝒆𝒔 𝑰𝑹𝑹𝑬𝑽𝑬𝑹𝑺𝑰𝑩𝑳𝑬!",
                reply_markup=reply_markup
            )
            await callback_query.answer()
            ...
            if not all_files:
                await callback_query.answer("📭 No hay archivos para subir.", show_alert=True)
                return
            ...
            await callback_query.message.edit_text(
                f"📦 **𝑨𝒓𝒄𝒉𝒊𝒗𝒐𝒔 𝒔𝒆𝒍𝒆𝒄𝒄𝒊𝒐𝒏𝒂𝒅𝒐𝒔:** {len(all_files)}\n"
                f"📦 𝑻𝒂𝒎𝒂ñ𝒐 𝒑𝒂𝒓𝒕𝒆𝒔: {CHUNK_SIZE_MB} MB\n\n"
                f"**𝑺𝒆𝒍𝒆𝒄𝒄𝒊𝒐𝒏𝒆 𝒍𝒂 𝒓𝒆𝒗𝒊𝒔𝒕𝒂 𝒅𝒆𝒔𝒕𝒊𝒏𝒐:**",
                reply_markup=reply_markup
            )
            ...
            if not selected_files:
                await callback_query.message.edit_text("📭 No hay archivos para subir.")
                await callback_query.answer()
                return
            ...
            await callback_query.message.edit_text(
                f"🚀 **𝑷𝒓𝒆𝒑𝒂𝒓𝒂𝒏𝒅𝒐 𝒔𝒖𝒃𝒊𝒅𝒂 𝒂 {revista_info['nombre']}**\n\n"
                f"📊 𝑨𝒓𝒄𝒉𝒊𝒗𝒐𝒔: {len(selected_files)}\n"
                f"📦 𝑻𝒂𝒎𝒂ñ𝒐 𝒑𝒂𝒓𝒕𝒆𝒔: {CHUNK_SIZE_MB} MB\n"
                f"🔑 𝑰𝒏𝒊𝒄𝒊𝒂𝒏𝒅𝒐 𝒔𝒆𝒔𝒊ó𝒏..."
            )
            ...
            if user_id not in user_selected_files:
                await callback_query.message.edit_text("❌ No hay archivos seleccionados.")
                await callback_query.answer()
                return
            ...
            await callback_query.message.edit_text(
                f"🚀 **𝑷𝒓𝒆𝒑𝒂𝒓𝒂𝒏𝒅𝒐 𝒔𝒖𝒃𝒊𝒅𝒂 𝒂 {revista_info['nombre']}**\n\n"
                f"📊 𝑨𝒓𝒄𝒉𝒊𝒗𝒐𝒔 𝒔𝒆𝒍𝒆𝒄𝒄𝒊𝒐𝒏𝒂𝒅𝒐𝒔: {len(selected_files)}\n"
                f"📦 𝑻𝒂𝒎𝒂ñ𝒐 𝒑𝒂𝒓𝒕𝒆𝒔: {CHUNK_SIZE_MB} MB\n"
                f"🔑 𝑰𝒏𝒊𝒄𝒊𝒂𝒏𝒅𝒐 𝒔𝒆𝒔𝒊ó𝒏..."
            )
            ...
            keyboard = [
                [
                    InlineKeyboardButton("✅ 𝑺í, 𝒆𝒍𝒊𝒎𝒊𝒏𝒂𝒓 𝒕𝒐𝒅𝒐", callback_data=f"confirm_clear_{revista_id}"),
                    InlineKeyboardButton("❌ 𝑪𝒂𝒏𝒄𝒆𝒍𝒂𝒓", callback_data="cancel_clear")
                ]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            
            await callback_query.message.edit_text(
                f"⚠️ **𝑪𝑶𝑵𝑭𝑰𝑹𝑴𝑨𝑪𝑰Ó𝑵 𝑹𝑬𝑸𝑼𝑬𝑹𝑰𝑫𝑨**\n\n"
                f"¿𝑬𝒔𝒕á 𝒔𝒆𝒈𝒖𝒓𝒐 𝒅𝒆 𝒆𝒍𝒊𝒎𝒊𝒏𝒂𝒓 𝑻𝑶𝑫𝑶𝑺 𝒍𝒐𝒔 𝒂𝒓𝒄𝒉𝒊𝒗𝒐𝒔 𝒅𝒆:\n"
                f"📚 **{revista_info['nombre']}**?\n\n"
                f"𝑬𝒔𝒕𝒂 𝒂𝒄𝒄𝒊ó𝒏 𝑵𝑶 𝒔𝒆 𝒑𝒖𝒆𝒅𝒆 𝒅𝒆𝒔𝒉𝒂𝒄𝒆𝒓.",
                reply_markup=reply_markup
            )
            ...
            await callback_query.message.edit_text(f"🔑 𝑰𝒏𝒊𝒄𝒊𝒂𝒏𝒅𝒐 𝒔𝒆𝒔𝒊ó𝒏 𝒆𝒏 {revista_info['nombre']}...")
            ...
            if user_id != ADMIN_ID:
                await callback_query.answer("❌ Solo el administrador puede configurar revistas.", show_alert=True)
                return
            ...
            keyboard.append([InlineKeyboardButton("❌ Cancelar", callback_data="close_panel")])
            reply_markup = InlineKeyboardMarkup(keyboard)
            ...
            await callback_query.message.edit_text(
                f"🔧 **𝑪𝒐𝒏𝒇𝒊𝒈𝒖𝒓𝒂𝒏𝒅𝒐: {revista_info['nombre']}**\n\n"
                f"📝 **𝑷𝒂𝒔𝒐 1 𝒅𝒆 3: 𝑵𝒐𝒎𝒃𝒓𝒆 𝒅𝒆 𝒖𝒔𝒖𝒂𝒓𝒊𝒐**\n\n"
                f"Nombre de usuario actual: `{revista_info['username']}`\n\n"
                f"𝑷𝒐𝒓 𝒇𝒂𝒗𝒐𝒓, 𝒆𝒏𝒗í𝒆 𝒆𝒍 𝒏𝒖𝒆𝒗𝒐 𝒏𝒐𝒎𝒃𝒓𝒆 𝒅𝒆 𝒖𝒔𝒖𝒂𝒓𝒊𝒐:"
            )
            await callback_query.answer("📝 Envíe el nombre de usuario...")
        
        elif data == "view_config":
            if user_id != ADMIN_ID:
                await callback_query.answer("❌ Solo el administrador puede ver la configuración.", show_alert=True)
                return
            
            config_text = "🔧 **𝑪𝑶𝑵𝑭𝑰𝑮𝑼𝑹𝑨𝑪𝑰Ó𝑵 𝑨𝑪𝑻𝑼𝑨𝑳**\n\n"
            
            for revista_id, revista_info in REVISTAS.items():
                config_text += f"📚 **{revista_info['nombre']}** ({revista_id})\n"
                config_text += f"• 𝑼𝒔𝒖𝒂𝒓𝒊𝒐: `{revista_info['username']}`\n"
                config_text += f"• 𝑺𝒖𝒃𝒎𝒊𝒔𝒔𝒊𝒐𝒏 𝑰𝑫: `{revista_info['submission_id']}`\n"
                config_text += f"• 𝑼𝑹𝑳: {revista_info['base_url']}\n"
                config_text += f"• 𝑪𝒐𝒏𝒕𝒆𝒙𝒕𝒐: {revista_info['contexto']}\n\n"
            
            config_text += f"📦 **𝑪𝒐𝒏𝒇𝒊𝒈𝒖𝒓𝒂𝒄𝒊ó𝒏 𝒈𝒆𝒏𝒆𝒓𝒂𝒍:**\n"
            config_text += f"• 𝑻𝒂𝒎𝒂ñ𝒐 𝒑𝒂𝒓𝒕𝒆𝒔: {CHUNK_SIZE_MB} MB\n"
            config_text += f"• 𝑫𝒊𝒓𝒆𝒄𝒕𝒐𝒓𝒊𝒐 𝒓𝒂í𝒛: {ROOT_DIR}\n"
        ...
        await callback_query.message.edit_text(f"❌ Error en callback: {str(e)}")
        await callback_query.answer("Error", show_alert=True)

# =========================
# MANEJADOR DE MENSAJES PARA CONFIGURACIÓN
# =========================
    ...
    try:
        ...
        if user_id in user_config_states:
            ...
            if revista_id not in REVISTAS:
                ...
                await message.reply("❌ Error: Revista no encontrada.")
                return
            ...
            if step == 'username':
                ...
                await message.reply(
                    f"✅ **𝑵𝒐𝒎𝒃𝒓𝒆 𝒅𝒆 𝒖𝒔𝒖𝒂𝒓𝒊𝒐 𝒂𝒄𝒕𝒖𝒂𝒍𝒊𝒛𝒂𝒅𝒐**\n\n"
                    f"📝 **𝑷𝒂𝒔𝒐 2 𝒅𝒆 3: 𝑪𝒐𝒏𝒕𝒓𝒂𝒔𝒆ñ𝒂**\n\n"
                    f"Contraseña actual: `{revista_info['password']}`\n\n"
                    f"𝑷𝒐𝒓 𝒇𝒂𝒗𝒐𝒓, 𝒆𝒏𝒗í𝒆 𝒍𝒂 𝒏𝒖𝒆𝒗𝒂 𝒄𝒐𝒏𝒕𝒓𝒂𝒔𝒆ñ𝒂:"
                )
                ...
                await message.reply(
                    f"✅ **𝑪𝒐𝒏𝒕𝒓𝒂𝒔𝒆ñ𝒂 𝒂𝒄𝒕𝒖𝒂𝒍𝒊𝒛𝒂𝒅𝒂**\n\n"
                    f"📝 **𝑷𝒂𝒔𝒐 3 𝒅𝒆 3: 𝑺𝒖𝒃𝒎𝒊𝒔𝒔𝒊𝒐𝒏 𝑰𝑫**\n\n"
                    f"Submission ID actual: `{revista_info['submission_id']}`\n\n"
                    f"𝑷𝒐𝒓 𝒇𝒂𝒗𝒐𝒓, 𝒆𝒏𝒗í𝒆 𝒆𝒍 𝒏𝒖𝒆𝒗𝒐 𝑺𝒖𝒃𝒎𝒊𝒔𝒔𝒊𝒐𝒏 𝑰𝑫:"
                )
                ...
                if save_revistas_config(REVISTAS):
                    ...
                    await message.reply(
                        f"🎉 **¡𝑪𝒐𝒏𝒇𝒊𝒈𝒖𝒓𝒂𝒄𝒊ó𝒏 𝒄𝒐𝒎𝒑𝒍𝒆𝒕𝒂𝒅𝒂!**\n\n"
                        f"📚 **𝑹𝒆𝒗𝒊𝒔𝒕𝒂:** {revista_info['nombre']}\n"
                        f"👤 **𝑼𝒔𝒖𝒂𝒓𝒊𝒐:** `{REVISTAS[revista_id]['username']}`\n"
                        f"🔑 **𝑷𝒂𝒔𝒔𝒘𝒐𝒓𝒅:** `{REVISTAS[revista_id]['password']}`\n"
                        f"📄 **𝑺𝒖𝒃𝒎𝒊𝒔𝒔𝒊𝒐𝒏 𝑰𝑫:** `{REVISTAS[revista_id]['submission_id']}`\n\n"
                        f"✅ 𝑳𝒂 𝒄𝒐𝒏𝒇𝒊𝒈𝒖𝒓𝒂𝒄𝒊ó𝒏 𝒉𝒂 𝒔𝒊𝒅𝒐 𝒈𝒖𝒂𝒓𝒅𝒂𝒅𝒂 𝒆𝒙𝒊𝒕𝒐𝒔𝒂𝒎𝒆𝒏𝒕𝒆."
                    )
                else:
                    await message.reply("❌ Error al guardar la configuración.")
                    
    except Exception as e:
        await message.reply(f"❌ Error en configuración: {str(e)}")
        if user_id in user_config_states:
            del user_config_states[user_id]

# =========================
# FUNCIONES DE SUBIDA CON DIVISIÓN DE ARCHIVOS
# =========================

async def perform_upload_with_chunking(client, message, user_id, revista_nombre, files_to_upload):
    """Realiza el proceso de subida CON DIVISIÓN DE ARCHIVOS GRANDES"""
    
    if user_id not in active_uploads:
        await message.edit_text("❌ Error: No hay uploader activo.")
        return
    ...
    try:
        await status_msg.edit_text("🔑 𝑰𝒏𝒊𝒄𝒊𝒂𝒏𝒅𝒐 𝒔𝒆𝒔𝒊ó𝒏 𝒆𝒏 𝒍𝒂 𝒓𝒆𝒗𝒊𝒔𝒕𝒂...")
        
        if not uploader.login():
            await status_msg.edit_text("❌ Error al iniciar sesión en la revista.")
            return

        await status_msg.edit_text(f"✅ 𝑳𝒐𝒈𝒊𝒏 𝒆𝒙𝒊𝒕𝒐𝒔𝒐\n📋 𝑺𝒖𝒃𝒎𝒊𝒔𝒔𝒊𝒐𝒏 𝑰𝑫: {uploader.submission_id}\n➡️ 𝑵𝒂𝒗𝒆𝒈𝒂𝒏𝒅𝒐 𝒂 𝒑𝒂𝒔𝒐 2...")
        
        if not uploader.navigate_to_step_2():
            await status_msg.edit_text("❌ No se pudo acceder al paso 2 del envío.")
            return
        ...
        with open(maestro_path, "w", encoding="utf-8") as maestro:
            ...
            maestro.write(f"# Tamaño partes: {CHUNK_SIZE_MB} MB\n\n")
            ...
            for idx, file_path in enumerate(files_to_upload, 1):
                ...
                if file_size > uploader.chunk_size:
                    ...
                    await status_msg.edit_text(
                        f"📚 **𝑹𝒆𝒗𝒊𝒔𝒕𝒂:** {revista_nombre}\n"
                        f"📂 [{idx}/{total_files}] {file_name}\n"
                        f"📦 𝑻𝒂𝒎𝒂ñ𝒐: {file_size_mb:.2f} MB → {total_parts} partes\n"
                        f"🔄 𝑷𝒓𝒆𝒑𝒂𝒓𝒂𝒏𝒅𝒐 𝒑𝒂𝒓𝒕𝒆𝒔...\n"
                        f"✅ 𝑺𝒖𝒃𝒊𝒅𝒐𝒔: {uploaded_count} | ❌ 𝑭𝒂𝒍𝒍𝒐𝒔: {failed_count}\n\n"
                        f"👨‍💻 𝑫𝒆𝒔𝒂𝒓𝒓𝒐𝒍𝒍𝒂𝒅𝒐𝒓: @Pro_Slayerr"
                    )
                    ...
                    if not parts:
                        await status_msg.edit_text(f"❌ Error al preparar partes para {file_name}")
                        failed_count += 1
                    ...
                    for part in parts:
                        ...
                        await status_msg.edit_text(
                            f"📤 **𝑺𝒖𝒃𝒊𝒆𝒏𝒅𝒐 𝒑𝒂𝒓𝒕𝒆 {part_idx}/{len(parts)}**\n"
                            f"📄 𝑨𝒓𝒄𝒉𝒊𝒗𝒐: {file_name}\n"
                            f"📦 𝑷𝒂𝒓𝒕𝒆: {part_name}\n"
                            f"⚡ 𝑷𝒓𝒐𝒈𝒓𝒆𝒔𝒐: 0%"
                        )
                        ...
                        if info:
                            ...
                            maestro.write(f"Tamaño: {part_size} bytes\n\n")
                            
                            all_urls.append(info['url'])
                            
                            await status_msg.edit_text(
                                f"✅ **𝑷𝒂𝒓𝒕𝒆 {part_idx}/{len(parts)} 𝒔𝒖𝒃𝒊𝒅𝒂**\n"
                                f"📄 𝑨𝒓𝒄𝒉𝒊𝒗𝒐: {file_name}\n"
                                f"📦 𝑷𝒂𝒓𝒕𝒆: {part_name}\n"
                                f"⚡ 𝑷𝒓𝒐𝒈𝒓𝒆𝒔𝒐: {part_idx}/{len(parts)} 𝒑𝒂𝒓𝒕𝒆𝒔"
                            )
                        else:
                            await status_msg.edit_text(f"❌ Error al subir parte {part_idx} de {file_name}")
                            file_success = False
                            break
                    ...
                    try:
                        ...
                        print(f"⚠️ Error al limpiar directorio de partes: {e}")
                    
                    if file_success:
                        uploaded_count += 1
                        await status_msg.edit_text(f"✅ [{idx}/{total_files}] {file_name} completado ({len(parts)} partes)\n\n⏳ 𝑪𝒐𝒏𝒕𝒊𝒏𝒖𝒂𝒏𝒅𝒐...")
                    else:
                        failed_count += 1
                        await status_msg.edit_text(f"❌ [{idx}/{total_files}] {file_name} falló\n\n⏳ 𝑪𝒐𝒏𝒕𝒊𝒏𝒖𝒂𝒏𝒅𝒐...")
                        
                else:
                    await status_msg.edit_text(
                        f"📚 **𝑹𝒆𝒗𝒊𝒔𝒕𝒂:** {revista_nombre}\n"
                        f"📤 [{idx}/{total_files}] 𝑺𝒖𝒃𝒊𝒆𝒏𝒅𝒐: {file_name}\n"
                        f"📦 𝑻𝒂𝒎𝒂ñ𝒐: {file_size_mb:.2f} MB\n"
                        f"✅ 𝑺𝒖𝒃𝒊𝒅𝒐𝒔: {uploaded_count} | ❌ 𝑭𝒂𝒍𝒍𝒐𝒔: {failed_count}\n\n"
                        f"⏳ 𝑷𝒐𝒓 𝒇𝒂𝒗𝒐𝒓 𝒆𝒔𝒑𝒆𝒓𝒂..."
                    )
                    ...
                    if not file_path.lower().endswith(".pdf"):
                        ...
                        if fake_path:
                            ...
                            print(f"✅ Archivo renombrado a PDF: {file_name}")
                        else:
                            await status_msg.edit_text(f"❌ No se pudo renombrar {file_name}")
                            failed_count += 1
                    ...
                    if info:
                        ...
                        maestro.write(f"URL: {info['url']}\n")
                        maestro.write(f"Tamaño: {file_size} bytes\n\n")
                        ...
                        await status_msg.edit_text(f"✅ [{idx}/{total_files}] {file_name} subido\n\n⏳ 𝑪𝒐𝒏𝒕𝒊𝒏𝒖𝒂𝒏𝒅𝒐...")
                    else:
                        failed_count += 1
                        await status_msg.edit_text(f"❌ [{idx}/{total_files}] Error al subir {file_name}\n\n⏳ 𝑪𝒐𝒏𝒕𝒊𝒏𝒖𝒂𝒏𝒅𝒐...")
                        maestro.write(f"Archivo: {file_name} - ERROR al subir\n\n")
                
                await asyncio.sleep(1)
            
            await status_msg.edit_text(
                f"✅ **𝑷𝒓𝒐𝒄𝒆𝒔𝒐 𝒅𝒆 𝒔𝒖𝒃𝒊𝒅𝒂 𝒇𝒊𝒏𝒂𝒍𝒊𝒛𝒂𝒅𝒐**\n\n"
                f"📚 𝑹𝒆𝒗𝒊𝒔𝒕𝒂: {revista_nombre}\n"
                f"📦 𝑻𝒐𝒕𝒂𝒍 𝒂𝒓𝒄𝒉𝒊𝒗𝒐𝒔: {total_files}\n"
                f"✅ 𝑺𝒖𝒃𝒊𝒅𝒐𝒔 𝒆𝒙𝒊𝒕𝒐𝒔𝒂𝒎𝒆𝒏𝒕𝒆: {uploaded_count}\n"
                f"❌ 𝑭𝒂𝒍𝒍𝒐𝒔: {failed_count}\n"
                f"📄 𝑮𝒆𝒏𝒆𝒓𝒂𝒏𝒅𝒐 𝒂𝒓𝒄𝒉𝒊𝒗𝒐 𝒎𝒂𝒆𝒔𝒕𝒓𝒐..."
            )
        
        if all_urls:
            urls_text = f"🔗 **𝑼𝑹𝑳𝒔 𝒔𝒖𝒃𝒊𝒅𝒂𝒔 𝒂 {revista_nombre}**\n\n"
            urls_text += f"📊 𝑻𝒐𝒕𝒂𝒍 𝑼𝑹𝑳𝒔: {len(all_urls)}\n\n"
            ...
            if len(urls_text) > 4000:
                ...
                await status_msg.reply(f"📄 **𝑵𝒐𝒕𝒂:** Se encontraron {len(all_urls)} URLs en total.")
            else:
        ...
        if os.path.exists(maestro_path):
            await client.send_document(
                ...
                caption=(
                    f"📄 **𝑨𝒓𝒄𝒉𝒊𝒗𝒐 𝒎𝒂𝒆𝒔𝒕𝒓𝒐 - {revista_nombre}**\n"
                    f"📦 𝑻𝒂𝒎𝒂ñ𝒐 𝒑𝒂𝒓𝒕𝒆𝒔: {CHUNK_SIZE_MB} MB\n"
                    f"✅ {uploaded_count} 𝒂𝒓𝒄𝒉𝒊𝒗𝒐𝒔 𝒔𝒖𝒃𝒊𝒅𝒐𝒔 𝒆𝒙𝒊𝒕𝒐𝒔𝒂𝒎𝒆𝒏𝒕𝒆\n"
                    f"❌ {failed_count} 𝒂𝒓𝒄𝒉𝒊𝒗𝒐𝒔 𝒇𝒂𝒍𝒍𝒂𝒓𝒐𝒏\n"
                    f"📋 𝑻𝒐𝒕𝒂𝒍 𝒑𝒓𝒐𝒄𝒆𝒔𝒂𝒅𝒐𝒔: {total_files}\n"
                    f"🔗 𝑼𝑹𝑳𝒔 𝒕𝒐𝒕𝒂𝒍𝒆𝒔: {len(all_urls)}\n"
                    f"👨‍💻 𝑫𝒆𝒔𝒂𝒓𝒓𝒐𝒍𝒍𝒂𝒅𝒐𝒓: @Pro_Slayerr"
                )
            )
        
        await status_msg.edit_text(
            f"🎉 **¡𝑷𝒓𝒐𝒄𝒆𝒔𝒐 𝒄𝒐𝒎𝒑𝒍𝒆𝒕𝒂𝒅𝒐!**\n\n"
            f"📚 𝑹𝒆𝒗𝒊𝒔𝒕𝒂: {revista_nombre}\n"
            f"📦 𝑨𝒓𝒄𝒉𝒊𝒗𝒐𝒔 𝒑𝒓𝒐𝒄𝒆𝒔𝒂𝒅𝒐𝒔: {total_files}\n"
            f"✅ 𝑺𝒖𝒃𝒊𝒅𝒐𝒔 𝒆𝒙𝒊𝒕𝒐𝒔𝒂𝒎𝒆𝒏𝒕𝒆: {uploaded_count}\n"
            f"❌ 𝑭𝒂𝒍𝒍𝒐𝒔: {failed_count}\n"
            f"🔗 𝑼𝑹𝑳𝒔 𝒈𝒆𝒏𝒆𝒓𝒂𝒅𝒂𝒔: {len(all_urls)}\n"
            f"📄 𝑹𝒆𝒗𝒊𝒔𝒂 𝒍𝒂𝒔 𝑼𝑹𝑳𝒔 𝒚 𝒆𝒍 𝒂𝒓𝒄𝒉𝒊𝒗𝒐 𝒎𝒂𝒆𝒔𝒕𝒓𝒐.\n\n"
            f"👨‍💻 𝑫𝒆𝒔𝒂𝒓𝒓𝒐𝒍𝒍𝒂𝒅𝒐𝒓: @Pro_Slayerr"
        )
        
    except Exception as e:
        await status_msg.edit_text(f"❌ Error en la subida: {str(e)}")
...
async def clear_revista_files(client, message, uploader, revista_nombre):
    ...
    try:
        await status_msg.edit_text("🔑 𝑰𝒏𝒊𝒄𝒊𝒂𝒏𝒅𝒐 𝒔𝒆𝒔𝒊ó𝒏...")
        if not uploader.login():
            await status_msg.edit_text("❌ Error al iniciar sesión.")
            return
        
        await status_msg.edit_text("📋 𝑶𝒃𝒕𝒆𝒏𝒊𝒆𝒏𝒅𝒐 𝒍𝒊𝒔𝒕𝒂 𝒅𝒆 𝒂𝒓𝒄𝒉𝒊𝒗𝒐𝒔...")
        files = uploader.get_submission_files()
        
        if not files:
            await status_msg.edit_text(f"📭 No hay archivos en {revista_nombre}.")
            return
        ...
        await status_msg.edit_text(
            f"🗑️ **𝑳𝒊𝒎𝒑𝒊𝒂𝒏𝒅𝒐 {revista_nombre}**\n\n"
            f"📊 𝑨𝒓𝒄𝒉𝒊𝒗𝒐𝒔 𝒆𝒏𝒄𝒐𝒏𝒕𝒓𝒂𝒅𝒐𝒔: {total_files}\n"
            f"⏳ 𝑬𝒍𝒊𝒎𝒊𝒏𝒂𝒏𝒅𝒐..."
        )
        ...
        for i, file_info in enumerate(files, 1):
            ...
            if file_id:
                ...
                if i % 5 == 0:
                    await status_msg.edit_text(
                        f"🗑️ **𝑳𝒊𝒎𝒑𝒊𝒂𝒏𝒅𝒐 {revista_nombre}**\n\n"
                        f"📊 𝑷𝒓𝒐𝒈𝒓𝒆𝒔𝒐: {i}/{total_files}\n"
                        f"✅ 𝑬𝒍𝒊𝒎𝒊𝒏𝒂𝒅𝒐𝒔: {deleted_count}"
                    )
                
                await asyncio.sleep(1)
        
        await status_msg.edit_text(
            f"✅ **𝑳𝒊𝒎𝒑𝒊𝒆𝒛𝒂 𝒄𝒐𝒎𝒑𝒍𝒆𝒕𝒂𝒅𝒂**\n\n"
            f"📚 𝑹𝒆𝒗𝒊𝒔𝒕𝒂: {revista_nombre}\n"
            f"🗑️ 𝑨𝒓𝒄𝒉𝒊𝒗𝒐𝒔 𝒆𝒍𝒊𝒎𝒊𝒏𝒂𝒅𝒐𝒔: {deleted_count}/{total_files}\n"
            f"🕐 {time.strftime('%Y-%m-%d %H:%M:%S')}"
        )
        
    except Exception as e:
        await status_msg.edit_text(f"❌ Error en la limpieza: {str(e)}")

# =========================
# INICIAR BOT
# =========================
print("=" * 60)
print("🚀 𝑩𝑶𝑻 𝑫𝑬 𝑮𝑬𝑺𝑻𝑰Ó𝑵 𝑫𝑬 𝑹𝑬𝑽𝑰𝑺𝑻𝑨𝑺 - 𝑰𝑵𝑰𝑪𝑰𝑨𝑵𝑫𝑶")
print("=" * 60)
print(f"✅ 𝑩𝒐𝒕 𝑻𝒐𝒌𝒆𝒏: {'*' * 20}{BOT_TOKEN[-5:]}")
print(f"✅ 𝑨𝑷𝑰 𝑰𝑫: {API_ID}")
print(f"👑 𝑨𝒅𝒎𝒊𝒏𝒊𝒔𝒕𝒓𝒂𝒅𝒐𝒓 𝑰𝑫: {ADMIN_ID}")
print(f"📂 𝑫𝒊𝒓𝒆𝒄𝒕𝒐𝒓𝒊𝒐 𝒓𝒂í𝒛: {ROOT_DIR}")
print(f"📦 𝑻𝒂𝒎𝒂ñ𝒐 𝒅𝒆 𝒑𝒂𝒓𝒕𝒆𝒔: {CHUNK_SIZE_MB} MB")
print(f"📚 𝑹𝒆𝒗𝒊𝒔𝒕𝒂𝒔 𝒄𝒐𝒏𝒇𝒊𝒈𝒖𝒓𝒂𝒅𝒂𝒔: {len(REVISTAS)}")
print(f"👥 𝑼𝒔𝒖𝒂𝒓𝒊𝒐𝒔 𝒂𝒖𝒕𝒐𝒓𝒊𝒛𝒂𝒅𝒐𝒔: {len(load_authorized_users())}")

print("\n" + "=" * 60)
print("📋 𝑪𝑶𝑴𝑨𝑵𝑫𝑶𝑺 𝑪𝑶𝑵𝑭𝑰𝑮𝑼𝑹𝑨𝑫𝑶𝑺:")
print("=" * 60)
print("🔓 𝑷Ú𝑩𝑳𝑰𝑪𝑶𝑺 (𝒔𝒊𝒏 𝒂𝒖𝒕𝒐𝒓𝒊𝒛𝒂𝒄𝒊ó𝒏):")
print("  • /start - Iniciar bot")
print("  • /help - Ayuda completa")
print("\n👑 𝑺𝑶𝑳𝑶 𝑨𝑫𝑴𝑰𝑵𝑰𝑺𝑻𝑹𝑨𝑫𝑶𝑹:")
print("  • /add <id/@user> - Agregar usuario")
print("  • /ban <id> - Eliminar usuario")
print("  • /users - Listar usuarios")
print("  • /settings - Panel de configuración")
print("\n🔒 𝑼𝑺𝑼𝑨𝑹𝑰𝑶𝑺 𝑨𝑼𝑻𝑶𝑹𝑰𝒁𝑨𝑫𝑶𝑺:")
print("  • /ls - Listar archivos (𝒊𝒏𝒕𝒆𝒓𝒇𝒂𝒛 𝒎𝒐𝒅𝒆𝒓𝒏𝒂)")
print("  • /rm <número> - Eliminar archivo")
print("  • /rm_<número> - Eliminar archivo (nuevo formato)")
print("  • /deleteall - Eliminar todo")
print("  • /up - Subir archivos (𝑪𝑶𝑵 𝑫𝑰𝑽𝑰𝑺𝑰Ó𝑵)")
print("  • /up_<número> - Subir archivo específico")
print("  • /zips <MB> - Cambiar tamaño partes")
print("  • /clear_rev - Limpiar revista")

print("\n" + "=" * 60)
print("🆕 **𝑵𝑼𝑬𝑽𝑨𝑺 𝑪𝑨𝑹𝑨𝑪𝑻𝑬𝑹Í𝑺𝑻𝑰𝑪𝑨𝑺:**")
print("   • 𝑰𝒏𝒕𝒆𝒓𝒇𝒂𝒛 𝒎𝒐𝒅𝒆𝒓𝒏𝒊𝒛𝒂𝒅𝒂 𝒆𝒏 /ls")
print("   • 𝑺𝒐𝒑𝒐𝒓𝒕𝒆 𝒑𝒂𝒓𝒂 /up_<número>")
print("   • 2 𝒃𝒐𝒕𝒐𝒏𝒆𝒔: 𝑺𝒖𝒃𝒊𝒓 𝑻𝒐𝒅𝒐/𝑬𝒍𝒊𝒎𝒊𝒏𝒂𝒓 𝑻𝒐𝒅𝒐")
print("   • 𝑻𝒊𝒆𝒎𝒑𝒐 𝒅𝒆 𝒂𝒄𝒕𝒊𝒗𝒊𝒅𝒂𝒅 𝒅𝒆𝒍 𝒃𝒐𝒕")
print("=" * 60)
print("\n🔄 **𝑫𝑰𝑽𝑰𝑺𝑰Ó𝑵 𝑫𝑬 𝑨𝑹𝑪𝑯𝑰𝑽𝑶𝑺 𝑨𝑪𝑻𝑰𝑽𝑨𝑫𝑨**")
print(f"   𝑨𝒓𝒄𝒉𝒊𝒗𝒐𝒔 > {CHUNK_SIZE_MB} MB 𝒔𝒆 𝒅𝒊𝒗𝒊𝒅𝒊𝒓á𝒏 𝒂𝒖𝒕𝒐𝒎á𝒕𝒊𝒄𝒂𝒎𝒆𝒏𝒕𝒆")
print("=" * 60)
print("\n👨‍💻 𝑫𝒆𝒔𝒂𝒓𝒓𝒐𝒍𝒍𝒂𝒅𝒐𝒓: @Pro_Slayerr")
print("=" * 60)
print("\n🔄 𝑩𝒐𝒕 𝒆𝒏 𝒆𝒋𝒆𝒄𝒖𝒄𝒊ó𝒏...")
